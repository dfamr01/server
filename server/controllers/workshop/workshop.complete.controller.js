    ...
    try {
        ...
        if (isPaid(workshop.paymentType)) {
            if (!user.stripeAccountId) {
                return res
                    .status(401)
                    .jsend.fail(new Error("Please connect to stripe first"), { error: CONNECT_TO_STRIPE });
            }

            if (user.stripeAccountStatus !== USER_STRIPE_ACCOUNT_STATUS.VERIFIED.key) {
                return res
                    .status(401)
                    .jsend.fail(new Error("Please finish verify your Stripe account"), {
                        error: FINISH_STRIPE_ACCOUNT,
                    });
            }
        }
