...
const {validateRequest, getAllParts, createFileKey} = require('./helpers/upload.video.utils');
  ...
  busboy.on('file', async function (name, file, info) {
    ...
    try {
      ...
      if (!partExist) {
        ...
        await dbPart.save();
        logger.info(`added to db: flowChunkNumber, flowTotalChunks`, Number(flowChunkNumber), Number(flowTotalChunks));
