...
function createFileKey(eventId, fileName) {
  if (!eventId || !fileName) {
    throw new Error('eventId and fileName are required.');
  }
  return `event_${eventId}/file_${fileName}`;
}
...
async function checkIfChunkExists(s3, params, flowChunkNumber, flowCurrentChunkSize, nextPartNumberMarker) {
  logger.info('checkIfChunkExists params, chunkNumber, nextPartNumberMarker', params, flowChunkNumber, nextPartNumberMarker);
  try {
    ...
    logger.info('checkIfChunkExists Parts, IsTruncated, NextPartNumberMarker', Parts, IsTruncated, NextPartNumberMarker);
