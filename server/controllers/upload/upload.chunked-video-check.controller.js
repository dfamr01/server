...
const {validateRequest, checkIfChunkExists, createFileKey} = require('./helpers/upload.video.utils');
...
exports.uploadChunkedVideoCheck = async function (req, res, next) {
  try {
    ...
    if (validation === 'valid') {
      ...
      const chunkExists = await checkIfChunkExists(s3, params, Number(flowChunkNumber), Number(flowCurrentChunkSize));
