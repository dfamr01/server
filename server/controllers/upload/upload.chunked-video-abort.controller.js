const {AbortMultipartUploadCommand} = require("@aws-sdk/client-s3");
const logger = require('../../config/log4js')('upload-chunked-video-abort-ctrl');

const {isAdminOrOwner} = require('../../shared/permissions/middleware/upload');
const Event = require('../../shared/database/models/event.model');
const config = require('../../config/config');
const {getS3Client} = require("../../shared/config/aws");
const {UploadMultiPart} = require("../../shared/database/models");
const {createFileKey} = require("./helpers/upload.video.utils");

exports.uploadChunkedVideoAbort = async function (req, res, next) {
  try {
    const {EventId, fileName, uniqueIdentifier} = req.body;
    logger.info('fileName, uniqueIdentifier: ', fileName, uniqueIdentifier);

    const event = await Event.findByPk(EventId);

    if (!event) {
      return res.status(404).jsend.fail(new Error('Event was not found'));
    }

    if (!await isAdminOrOwner(req.user, event)) {
      return res.status(401).jsend.fail(new Error('You are unauthorised to perform this action'));
    }

    const s3 = getS3Client();

    let params = {
      Bucket: config.s3_bucket_name,
      Key: createFileKey(EventId, fileName),
      UploadId: uniqueIdentifier
    };

    UploadMultiPart.destroy({
      where: {
        EventId
      }
    })
    await s3.send(new AbortMultipartUploadCommand(params));

    logger.info('Aborted successfully');
    return res.status(200).jsend.success('aborted');
  } catch (e) {
    logger.info('Error: ', e);
    return res.status(500).jsend.error(new Error('Something went wrong.'));
  }
};

