...
const logger = require('../../config/log4js')('stripe.payment-new-charge');
...
exports.paymentNewCharge = async function (req, res, next) {
  try {
    const {user, body} = req;

    let {eventId, workshopId, occurrenceId, paymentMethodId} = body;

    if (!user.stripeCustomerId) {
      return res.status(404).jsend.fail(new Error('Stripe customerId does not exist'), {error: 'stripeCustomerNotExist'});
    }

    if (!eventId && !workshopId || !paymentMethodId) {
    ...
    if (boughtTickets.length) {
      return res.status(400).jsend.fail(new Error('You cannot buy the same content twice. \nPlease refresh the page'), {error: 'doublePurchase'});
    }
    ...
    const promises = reserveTickets.map(async (ticket) => {
      ...
      if (!intentId) {
        ...
        if (moment().subtract(1, 'hours').isAfter(moment(createdAt))) {
    ...
    await sequelize.transaction(async (t) => {
      ...
      logger.info('Success returning payment intent status to client');
