const logger = require('../../config/log4js')('admin-get-all-payouts');
const {PAGE_SIZE} = require('../../shared/config/constants');
const Payout = require('../../shared/database/models/payout.model');

exports.getAllPayouts = async function (req, res, next) {
  try {
    const {query} = req;
    let {orderBy, orderDir, page, pageSize, UserId} = query;

    const dbQuery = {
      where: {}
    };

    if (UserId) {
      dbQuery.where.UserId = UserId;
    }

    orderBy = orderBy || 'createdAt';
    orderDir = orderDir || 'ASC';

    dbQuery.order = [[orderBy, orderDir]];

    let _page = +page || 0;
    let _pageSize = +pageSize || PAGE_SIZE;

    dbQuery.offset = _page * _pageSize;
    dbQuery.limit = _pageSize;

    const rows = await Payout.findAll({...dbQuery});
    const count = await Payout.count(dbQuery);

    return res.status(200).jsend.success({
      payouts: {
        page: _page,
        pageSize: _pageSize,
        totalPages: Math.ceil(count / _pageSize),
        totalRows: count,
        rows
      }
    });
  } catch (err) {
    logger.warn(err.stack);
    return res.status(500).jsend.fail(err);
  }
};

